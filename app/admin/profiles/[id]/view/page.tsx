    ...
    return (
        <div className="container mx-auto py-8 px-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Original profile (with sepia tone) */}
                {isDraft && hasOriginal && originalProfile && (
                    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden relative sepia-container">
                        {/* Sepia overlay filter */}
                        <div className="absolute inset-0 bg-amber-50 dark:bg-amber-900 bg-opacity-25 dark:bg-opacity-15 pointer-events-none" />

                        <div className="p-6 relative z-10">
                            <div className="flex justify-between items-start mb-4">
                                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Original Published Version</h2>
                                <span className="px-2 py-1 bg-amber-100 text-amber-800 dark:bg-amber-900 dark:text-amber-200 text-xs rounded-md">
                                ...
                                <div>
                                    ...
                                    <div className="mt-2 grid grid-cols-2 sm:grid-cols-3 gap-2">
                                        {originalProfile.images.map((image, index) => (
                                            <div
                                                key={image.id}
                                                className={`relative aspect-[9/16] rounded-md overflow-hidden ${index === 0 ? 'ring-2 ring-amber-500' : ''}`}
                                            >
                                                <Image
                                                    src={image.mediumUrl}
                                                    alt={`Image ${index + 1}`}
                                                ...
                                                />
                                                {index === 0 && (
                                                    <div className="absolute top-1 left-1 bg-amber-600 text-white text-xs px-1.5 py-0.5 rounded-sm">
                                                        Main
                                                    </div>
                                                )}
                                            </div>
                    ...
                    <div className="p-6">
                        <div className="flex justify-between items-start mb-4">
                            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                                {isDraft && hasOriginal ? "New Draft Version" : "Profile Information"}
                            </h2>
                            {isDraft && (
